...
class HrPayslip(models.Model):
    ...
    def get_current_contract_dict(self, contract, contracts):
        ...
        :param contract: The current hr.version being processed
        :param contracts: Recordset of all hr.version records in this payslip
        :return: a dictionary of discreet values and/or Browsable Objects
        """
        self.ensure_one()
    ...
    def get_payslip_vals(
        ...
        if not self.env.context.get("contract"):
            ...
            if contract_id:
                ...
                # Odoo 19: _get_contracts returns defaultdict {employee_id: recordset}
                contracts_by_employee = employee._get_contracts(
                    date_start=date_from, date_end=date_to
                )
                contract_ids = contracts_by_employee.get(
                    employee.id, self.env["hr.version"]
                ).ids
        if not contract_ids:
            return res
    ...
    def _get_employee_contracts(self):
        ...
        for payslip in self:
            if payslip.contract_id.ids:
                ...
                # Odoo 19: _get_contracts returns defaultdict {employee_id: recordset}
                contracts_by_employee = payslip.employee_id._get_contracts(
                    date_start=payslip.date_from, date_end=payslip.date_to
                )
                contracts |= contracts_by_employee.get(
                    payslip.employee_id.id, self.env["hr.version"]
                )
        return contracts
