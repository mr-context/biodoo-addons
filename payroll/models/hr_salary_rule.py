...
# result = contract.wage * 0.10""",  # noqa: E501
    ...
    def _compute_rule(self, localdict):
        ...
        """
        self.ensure_one()
        method_name = f"_compute_rule_{self.amount_select}"
        method = getattr(self, method_name)
        return method(localdict)
...
"""
    ...
    def _satisfy_condition(self, localdict):
        ...
        """
        self.ensure_one()
        method_name = f"_satisfy_condition_{self.condition_select}"
        method = getattr(self, method_name)
        if self.parent_rule_id:
            current_result = method(localdict)
