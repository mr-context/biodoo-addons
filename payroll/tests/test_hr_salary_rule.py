...
class TestSalaryRule(TestPayslipBase):
    ...
    def test_python_code_return_values(self):
        ...
        # Ensure contract (hr.version) has a start date
        version = self.env["hr.version"].search([("employee_id", "=", self.richard_emp.id)], limit=1)
        if version and not version.contract_date_start:
            version.contract_date_start = self.env.fields.Date.today()
